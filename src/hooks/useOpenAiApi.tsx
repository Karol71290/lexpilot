...
export function useOpenAiApi() {
  ...
  // Add a new function to handle chat conversations
  const chatWithOpenAI = async (messages: ChatMessage[], options: OpenAIOptions = {}) => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      let response;
      try {
        console.log("Calling OpenAI chat with messages:", messages.length);
        console.log("Using model:", model);
      ...
      if (response.error) {
        const errorMessage = response.error.message || "Failed to generate response";
        console.error("Error calling OpenAI Chat API:", response.error);
        setError(errorMessage);
      ...
      const errorMessage = err instanceof Error ? err.message : "Unknown error occurred";
      console.error("Exception in chatWithOpenAI:", errorMessage);
      setError(errorMessage);
